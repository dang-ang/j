...
var GAS_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbwngOHZgwYZPbYsM8Yzfk-wNErmhVNVFvw0dzQWgkMKFbzaytkUhNo3-hiG3MZWluZs7A/exec";
...
function salvaCSV() {
  var squadra = document.getElementById("squadra").value || "Sconosciuta";
  var csv = "Squadra,Parametro,Minuto,Tempo\n";
  for (var i = 0; i < parametri.length; i++) {
    var codice = parametri[i].codice;
    var eventi = stats[codice];
    for (var j = 0; j < eventi.length; j++) {
      csv += squadra + "," + codice + "," + eventi[j].minuto + "," + eventi[j].tempo + "\n";
    }
  }
  for (var ruolo in vitalita) {
...
var link = document.createElement("a");
link.setAttribute("href", uri);
link.setAttribute("download", "nomefile.json"); // o .csv
document.body.appendChild(link);
link.click();
document.body.removeChild(link);
